...
hashmap<Key, Value, Compare, Hash>::erase(const Key& x) {
   ...
   //if the element is not found, it would be at the end of the container
   if (it != elems_[bucket].end()) {    
      auto tmp = it;
      ++it;

      elems_[bucket].erase(tmp);

      --size_;

      //if the iterator is now pointing to .end(), it was the last element
      //in that bucket, return iter to next bucket
      if(it == elems_[bucket].end()){
         ++bucket;
         //go until either there is a non-empty bucket or the end
         //of the vector
         while (elems_[bucket].empty() && bucket != elems_.size()){
            ++bucket; 
         }

         //if it is the end of the vector it is the last element in the hashmap
         if(bucket == elems_.size()) return make_pair(nullptr, true);

         //otherwise return the begining of the next bucket
         return make_pair(&(*elems_[bucket].begin()), true);
      }      

      //if it not pointing to end(), return a pointer to *it
      return make_pair(&(*it), true);
   }
...
void hashmap<Key, Value, Compare, Hash>::rehash(size_t n) {
   //to rehash, n must be larger than the current number of buckets
   if(n > hash_.numBuckets()){
      ...
      //populate tmp with the elements
      for(auto &elems : elems_)
         for(auto &e : elems)
            tmp.push_back(e);


      for(auto &t : tmp){
         size_t a = hash_.hash(t.first);
         elems_[a] = push_back(t);
      }
   }
